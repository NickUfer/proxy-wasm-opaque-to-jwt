use crate::jwt::jwt_producer::{Algorithm, JwtProducer};
...
impl Context for AuthHttpContext {
    fn on_http_call_response(
        ...
        match self.get_http_call_response_body(0, body_size) {
            ...
            Some(response_bytes) => {
                let value: Value = serde_json::from_slice(response_bytes.as_slice()).unwrap();
                ...
                self.add_http_request_header(
                    ...
                    self.jwt_producer.encode_jwt(value).as_str(),
                );
                self.resume_http_request();
            }
        }
        ...
        match self.get_configuration() {
            Some(config_bytes) => {
...

                {
                    ...
                    self.jwt_producer = JwtProducer::from(jwt_algorithm, cfg.jwt.key);
                }
                {
                    // TODO validation
